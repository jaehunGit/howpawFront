...
import { Dehaze, WindowTwoTone } from '@mui/icons-material';
...
import GoogleCode from './Component/GoogleCode';
import NaverCode from './Component/NaverCode';
...
export default function App(props) {
  ...
  const logout = () => {
    ...
    let ID = window.sessionStorage.removeItem("userId");
    let NICKNAME = window.sessionStorage.removeItem("nickName");
      ...
      <>
        <Box style={{width: 1400, height: 300, border: "1px solid black", zIndex: 100, position:"absolute", backgroundColor: "#ffffff",}}>
          <Grid container spacing={1}>
            ...
            <Grid item xs={3}>
              <List style={styles.listStyle} onClick={() => {window.sessionStorage.setItem("type", categoryEnum.SNACKS); window.location.href='ProductFilter';}}>
                <ListItemButton style={styles.listStyle.buttonTitleStyle}>
                  간식
                </ListItemButton>
                <ListItemButton style={styles.listStyle.buttonNomalStyle}>
    ...
    <BrowserRouter>
      ...
      style={{
        ...
        <Routes>
          ...
          <Route exact path="/login/oauth2/code/kakao" element={ <Kakao/> }/>
          <Route exact path="/KakaoSignUp" element={ <KakaoSignUp/> }/>
          <Route exact path="/login/oauth2/code/google" element={ <GoogleCode/> }/>
          <Route exact path="/login/oauth2/code/naver" element={ <NaverCode/> }/>
        </Routes>
      </div>
    </BrowserRouter>
  );
