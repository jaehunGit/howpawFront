...
export default function ProductFilterPage(props) {
  ...
  const showProduct = () => {
    return(
      <ImageList cols={3}>
        {productInfo && productInfo.map((item) => (
          parseInt(item[5].split(",")[0]) === tempType || parseInt(item[5].split(",")[1]) === tempType
          ?
          <ImageListItem key={item[2]}>
            <img
              src={item[2].replace("../client/public", ".")}
              srcSet={item[2].replace("../client/public", ".")}
              alt={item[1]}
              loading="lazy"
              onClick={ (e) => {window.sessionStorage.setItem("productID", item[0]); window.sessionStorage.setItem("productType", item[5]); navigate("../Product");}}
            />
            <ImageListItemBar
              title={item[1]}
              subtitle={<span>판매처: {item[4]}</span>}
              position="bottom"
            />
          </ImageListItem> : null
        )
