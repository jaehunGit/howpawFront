...
export default function ProductPage(props) {
  ...
  const getProductInfo = () => {
    axios.get("/api/GetProductInfo", {params: {pdcNumber: PdcNumber}}, )
    .then(res => {
        const body = res.data;

        getProductName(body.productName);
        getProductPrice(body.productPrice);
        getBrandName(body.brandName);
        getOrigin(body.origin);
        getWeight(body.productWeight);
        getProductLink(body.productLink);
        getBestReview(body.bestReviewText);
        getWorstReview(body.worstReviewText);
        getBestValue(body.bestRating);
        getWorstValue(body.worstRating);
        getMainImg(body.mainImageRoute);
        getDetailImg(body.detailImageRoute);
        getBestImg(body.bestImageRoute);
        getWorstImg(body.worstImageRoute);

        setMainImageSrc(body.mainImageRoute.replace("../client/public", "."));
        productMainIamge.current.src = body.mainImageRoute.replace("../client/public", ".");

        setDetailImageSrc(body.detailImageRoute.replace("../client/public", "."));
        productDetail.current.src = body.detailImageRoute.replace("../client/public", ".");
        
        setBestImageSrc(body.bestImageRoute.replace("../client/public", "."));
        bestImageRef.current.src = body.bestImageRoute.replace("../client/public", ".");

        setWorstImageSrc(body.worstImageRoute.replace("../client/public", "."));
        worstImageRef.current.src = body.worstImageRoute.replace("../client/public", ".");

        setTimeout(() => {
            detailImageSize.src = body.detailImageRoute.replace("../client/public", ".");
  ...
  const getSimilarProduct = () => {
    axios.get("/api/GetSimilarProduct", {params: {productType: productType, pdcNumber: PdcNumber}})
    .then(res => {
      const body = res.data;
      setRecomProductInfo(res.data);
    })
    .catch( res => console.log(res))
  }
  ...
  const getFavoriteProduct = () => {
    ...
    if ( window.sessionStorage.getItem("userId") != null ) {
      axios.get("/api/getFavoriteProduct", {params: { pdcNumber : PdcNumber, userId : window.sessionStorage.getItem("userId") }})
      .then( res => {
        const body = res.data;

        setFavoriteProduct(PdcNumber);
  ...
  const removeFavoriteProduct = () => {
    axios.post("/api/removeFavoriteProduct", {pdcNumber: PdcNumber, userId: window.sessionStorage.getItem("userId")})
    .then( res => {
  ...
  const addFavoriteProduct = () => {
    if(window.sessionStorage.getItem("userId") == null){
      ...
      axios.post("/api/AddFavoriteProduct", { pdcNumber : PdcNumber, userId : window.sessionStorage.getItem("userId") })
      .then( res => {
        const body = res.data;
        borderHeart.current.style.display="none";
        fullHeart.current.style.display="block";
          ...
          <div style={{width: "1200px", height: "400px"}}>
            <div ref={recomSec} style={{ width: "1100px", margin: "auto"}}>
              ...
              <StyledSlider {...settings} style={{width: "1000px", margin: "auto"}}>
                {
                  recomProductInfo && recomProductInfo.map((product) => (
                    <div key={product} onClick={ (e) => {window.sessionStorage.setItem("productID", product.pdcNumber); window.location.reload();}}>
                      <div style={{ width: "140px", height: "210px",marginRight: "20px"}}>
                        <img src={product.mainImageRoute.replace("../client/public", ".")} style={{ width: "100px", height: "100px", margin: "auto", marginTop: "10px", objectFit:"cover"}}></img>
                        <div style={{fontSize: "15px", height: "40px", overflow: "hidden"}}>{`${product.productName}`}</div>
                        <div style={{textAlign: "right"}}>{`${product.productPrice}원`}</div>
                        <Rating name="read-only" value={product.bestRating} style={{ top: "5px", left: "10px"}} readOnly/>
                      </div>
                    </div>
                  ))
                }
